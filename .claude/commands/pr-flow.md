...
### 4. OFFER MERGE (if final_status == "clean")

If final status is "clean":

Use AskUserQuestion:
```
{
  "questions": [{
    "question": "PR #{pr_number} is clean and ready. Merge now? (squash merge)",
    "header": "Merge PR",
    "multiSelect": false,
    "options": [
      {
...
PR needs manual review: https://github.com/user/repo/pull/43
Fix remaining issues or merge with caution.
```
