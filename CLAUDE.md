...
│   └── version-tag.yml      # Auto-tag version bumps
├── scripts/                  # Dev tooling
...
- **Pull Request to develop** → Deploys preview environment
...
**Usage:**
1. Go to Actions → Upload to Play Store
2. Click "Run workflow"
...
1. **Security Scanner** 🔒 - Auth checks, secrets, input validation, data exposure
2. **Standards Enforcer** 📋 - CLAUDE.md compliance, naming conventions, patterns
3. **Logic Analyzer** 🔍 - Bugs, error handling, edge cases, race conditions
4. **Performance Guardian** ⚡ - Rate limits, unbounded queries, memory leaks, cost control

**Severity Levels:**
- 🔴 **BLOCKING** - Cannot merge (security, broken functionality)
- 🟡 **HIGH** - Should fix (standards violations, missing error handling)
- 🟢 **MEDIUM** - Fix in follow-up (code clarity, minor refactoring)
- ℹ️ **INFO** - Optional (style suggestions, alternatives)
